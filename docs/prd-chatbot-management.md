...
4. **Broadcast** (`/admin/chatbot/broadcast`) — LINE broadcast/multicast + ตั้งเวลา (cron-job.org ยิง `api/cron/broadcast-send`)
...
- scheduler: **cron route** (`api/cron/broadcast-send`) แทน asyncio loop ของ jsk (serverless ไม่มี long-running process) — ตัวยิงคือ cron-job.org ไม่ใช่ Vercel Cron (ดู DEPLOY.md §4.3 job #5)
...
| Background task | asyncio loop (in-process) | cron routes (`api/cron/*`) ยิงจาก **cron-job.org** |
...
- jsk ใช้ asyncio loop → huangua ใช้ **cron-job.org** ยิง `api/cron/broadcast-send` ทุก 1 นาที + status guard `scheduled→sending` กันส่งซ้ำ (แบบ jsk `FOR UPDATE SKIP LOCKED`)
  - ห้ามใช้ Vercel Cron: Hobby จำกัดวันละครั้ง ใส่ `* * * * *` ลง `vercel.json` แล้ว deploy ล้มทั้งงานตั้งแต่ขั้นตรวจไฟล์ตั้งค่า
